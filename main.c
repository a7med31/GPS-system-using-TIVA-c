...
int main()
{
    ...
    //Enter destination coordinates here
    ...
    while(1)
    {
        memset(command,0,strlen(command));   //Empty the command array.
        getCommand(command);                 //Read the data from GPS module and store in command array.

        
        if (first == 0) {       //to store the starting coordinates of the GPS.
            getCoordinates();
            if(flag ==1){      //if the gps reading is not valid continue and take another reading.
                continue;
            }
            ...
            if(flag ==1){
             ...
             {
                total_distance += delta(p_lat, p_long, c_lat, c_long);   // total distance calculation by adding the distances calculated between each 2 consecutive readings.  
             }
             
        }


        //Condition to check the format is right first before getting distane to goal dtg.
		...
		if (dtg <= 0) { // if the destination is reached turn the green led on.
        ...
        else if (dtg < 5) { // if only 5m is left to reach the destination turn the yellow led on.
        ...
        else if (dtg > 5) { // if more than 5m is left to reach the destination turn the red led on.
